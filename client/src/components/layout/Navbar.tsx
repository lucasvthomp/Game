...
export default function Navbar() {
  ...
  const links = [
    { href: "/como-funciona", label: "Explorar" },
  ...
  return (
    <nav className="nav-root">
      ...
      {open && <div className="nav-mobile">{user && <div className="nav-mobile-profile"><div className="nav-avatar-dot">{user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : user.fullName[0]}</div><span><strong>{user.fullName}</strong><small>{isCaptain ? "Capitão Marcamar" : "Passageiro"}</small></span></div>}{links.map((link) => <Link key={link.href} href={link.href}><span className={"nav-mobile-link " + (isActive(link.href) ? "active" : "")} onClick={() => setOpen(false)}>{link.label}</span></Link>)}{user && <><Link href="/perfil"><span className="nav-mobile-link" onClick={() => setOpen(false)}>Perfil</span></Link><Link href="/notificacoes"><span className="nav-mobile-link" onClick={() => setOpen(false)}>Notificações{unreadNotifications > 0 ? ` (${unreadNotifications})` : ""}</span></Link></>}<div className="nav-mobile-divider" />{user ? <button className="nav-mobile-link" style={{ background: "none", border: "none", width: "100%", textAlign: "left", color: "var(--text2)", cursor: "pointer" }} onClick={() => { logout(); setOpen(false); }}>Sair</button> : <div className="nav-mobile-actions"><Link href="/entrar"><span className="nav-mobile-btn" onClick={() => setOpen(false)}>Entrar</span></Link><Link href="/cadastro"><span className="nav-mobile-btn" style={{ background: "var(--boat)", color: "#fff", fontWeight: 700 }} onClick={() => setOpen(false)}>Cadastrar</span></Link></div>}</div>}
    </nav>
  );
}
