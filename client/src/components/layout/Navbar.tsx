    ...
    <nav className="nav-root">
      ...
      <div className="nav-inner">
        ...
        <div className="nav-auth hidden md-flex">
          ...
          {user ? <><Link href="/perfil"><div className="nav-avatar"><div className="nav-avatar-dot">{user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : user.fullName[0]}</div><span>{user.fullName.split(" ")[0]}</span></div></Link><button className="nav-logout" onClick={() => logout()}>Sair</button></> : <><Link href="/entrar"><span className="nav-login">Entrar</span></Link><Link href="/cadastro"><span className="nav-signup">Cadastrar</span></Link></>}
        </div>
